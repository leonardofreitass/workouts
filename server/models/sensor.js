...
const findByParticipants = participants =>
  Promise.resolve(_.filter(sensor => sensor.usable &&
      _.includes(sensor.owner, participants))(SensorsMock));

const findAvailable = () =>
  Promise.resolve(_.find({ usable: true, owner: null }, SensorsMock));

const findAvailableForParticipants = count =>
  Promise.resolve(_.pipe(
